...
tab1, tab2, tab3 = st.tabs(['**账号**', '**视频**', '**评论**'])
...
with tab2:
    ...
    st.write(
        f"平均点赞数 **{int(np.mean(video_data['点赞数'])):,}**，最低点赞数 **{min(video_data['点赞数']):,}**，最高点赞数 **{max(video_data['点赞数']):,}**，点赞量中位数 **{int(np.median(video_data['点赞数'])):,}**")
    st.write("更新频率：**一日多更**")
    st.divider()
    st.header('视频名称词云')
